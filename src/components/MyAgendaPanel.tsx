...
const MyAgendaPanel: React.FC<MyAgendaPanelProps> = ({ activities, onClose, onAttendChange }) => {
  ...
  const handleShareWhatsApp = () => {
    ...
    const lines: string[] = ['📅 *Mi plan en Barcelona:*', ''];
    ...
    lines.push('¡Apúntate a los que quieras! 🙌');
        ...
        <div>
          ...
          <div style={{ fontSize: '0.75rem', opacity: 0.85, marginTop: 2 }}>
            {agendaActivities.length === 0
              ? 'Sin actividades añadidas'
              : `${agendaActivities.length} actividad${agendaActivities.length !== 1 ? 'es' : ''}`}
          </div>
        </div>
        <button
          onClick={onClose}
        ...
        {agendaActivities.length === 0 ? (
          <div style={{
            ...
            <p style={{ margin: 0, fontSize: '0.9rem', lineHeight: 1.5 }}>
              Marca 🙋‍♂️ en las actividades que quieras asistir y aparecerán aquí.
            </p>
          </div>
        ) : (
      ...
      {agendaActivities.length > 0 && (
        <div style={{ padding: '0.75rem 1rem', borderTop: '1px solid #e5e7eb', flexShrink: 0 }}>
          ...
          <p style={{ fontSize: '0.68rem', color: '#9ca3af', textAlign: 'center', margin: '0.4rem 0 0', lineHeight: 1.4 }}>
            Se abrirá WhatsApp con tu lista de planes
          </p>
        </div>
      )}
    </div>
